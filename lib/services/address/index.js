...
AddressService.prototype.getAddressSummary = function(address, options, callback) {
  ...
  options.from = options.from || 0;
  options.to = options.to || 0xffffffff;
  options.confirmations = options.confirmations || 3;
...
AddressService.prototype._setOutputResults = function(tx, address, result, options) {
  ...
  for(var j = 0; j < tx.outputs.length; j++) {
    ...
    if (tx.confirmations <= options.confirmations) {
...
AddressService.prototype._setInputResults = function(tx, address, result, options) {
...
AddressService.prototype._getAddressSummaryResult = function(txs, address, result, options) {
  ...
  for(var i = 0; i < txs.length; i++) {
    ...
    self._setOutputResults(tx, address, result, options);
    self._setInputResults(tx, address, result, options);
